        ...
        return [
            ...
            },
            {
                ...
                "check": lambda emr, info: (
                    not info.get("duration") or
                    (info.get("duration") and info.get("duration") in emr.subjective)
                ),
    ...
    async def check(
        ...
        try:
            ...
            rule_issues, llm_report = await asyncio.gather(
                asyncio.to_thread(
                    self._check_rules, emr, medical_info_dict
                ),
                self._check_with_llm(emr, medical_info_dict),
            ...
            score = self._calculate_score(
                emr, all_issues, medical_info_dict
            )
    ...
    async def _check_with_llm(
        ...
        try:
            ...
            messages = [
                ...
                {"role": "user", "content": f"""请检查以下SOAP病历：
