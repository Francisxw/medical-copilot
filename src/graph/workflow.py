...
class MedicalCopilotWorkflow:
    ...
    def __init__(
        ...
        # 根据配置选择检索方式
        mode = (retrieval_mode or settings.retrieval_mode or "simple").lower()
        if use_vector_retrieval:
            mode = "vector"
        elif (
            os.getenv("USE_VECTOR_RETRIEVAL", "false").lower() == "true"
            and mode == "simple"
        ):
            mode = "vector"

        if mode not in {"simple", "vector", "llamagraph"}:
        ...
        if mode == "llamagraph":
            ...
            self.retrieval_agent = LlamaGraphRAGAgent()
            logger.info("✅ 使用 LlamaIndex GraphRAG 检索模式")
        elif mode == "vector":
            from src.agents.retrieval_agent_vector import VectorRetrievalAgent

            self.retrieval_agent = VectorRetrievalAgent()
            logger.info("✅ 使用向量检索模式")
        else:
            self.retrieval_agent = SimpleRetrievalAgent()
            logger.info("✅ 使用关键词检索模式")
    ...
    async def run(self, inputs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            # 运行工作流
            # type: ignore - LangGraph 类型提示不完善，但 ainvoke 方法确实存在
